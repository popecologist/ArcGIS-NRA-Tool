...
namespace SpeciesTool.NET
{
    public partial class SelectionForm : Form
    {
        ...
        private static List<string> GetSelectedValues(string a_Caption, IEnumerable<string> a_Layers)
        {
            using (var a_Form = new SelectionForm())
            {
                a_Form.lbCaption.Text = a_Caption;
                var a_SelectedLayers = new List<string>();
                a_Form.chlbItems.Items.Clear();
                foreach (var a_Layer in a_Layers)
                    a_Form.chlbItems.Items.Add(a_Layer);
                if (a_Form.ShowDialog() != DialogResult.Cancel)
                    a_SelectedLayers.AddRange(a_Form.chlbItems.CheckedItems.OfType<string>());
                return a_SelectedLayers;
            }
        }

        public static List<string> GetSelectedLayers(IEnumerable<string> a_Layers)
        {
            return GetSelectedValues("Select the layers species", a_Layers);
        }

        public static List<string> GetSelectedGroups(IEnumerable<string> a_Groups)
        {
            return GetSelectedValues("Select the calculations", a_Groups);
        }
    }
}
